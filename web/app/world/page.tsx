...
export default function WorldPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('oracle.mobileAtlas');
    if (saved === 'toggle' || saved === 'stack' || saved === 'sheet') setMobileAtlas(saved);
  }, []);
  const setAtlasMode = (m: AtlasMode) => {
    setMobileAtlas(m);
    localStorage.setItem('oracle.mobileAtlas', m);
  };
  ...
  useEffect(() => {
    ...
    // re-bind when the map node remounts (e.g. mobile Map/Roster toggle)
  }, [vp.isMobile, mobileAtlas]);
  ...
  // ── the framed map plate (shared by desktop + every mobile layout) ──
  const mapFrame = (outerStyle: CSSProperties, compact: boolean): ReactNode => (
    <div style={outerStyle}>
      <div style={{ position: 'absolute', inset: 5, border: `1px solid ${c.line}`, overflow: 'hidden' }}>
        ...
        {/* gilt rings — the god's eye (desktop only; mobile frames stay clean) */}
        {!compact && (
        ...
        {/* zoom controls — top-right when compact (mobile), else bottom-right */}
        <div style={{ position: 'absolute', right: 10, ...(compact ? { top: 10 } : { bottom: 10 }), display: 'flex', flexDirection: 'column', gap: 4, zIndex: 4 }}>
  ...
  const renderMap = ({ fill, height }: { fill: boolean; height?: number | string }): ReactNode =>
    mapFrame(
      fill
        ? { position: 'absolute', inset: 0, border: `1px solid ${c.lineStrong}`, boxShadow: '0 2px 18px rgba(0,0,0,0.35)' }
        : { position: 'relative', height, border: `1px solid ${c.lineStrong}`, boxShadow: '0 2px 18px rgba(0,0,0,0.35)' },
      true,
    );
  ...
  const selectionKey = selectedAgent?.id ?? selectedDeadAgent?.id ?? null;
  const sheetTitle =
    selectedAgent !== null
      ? `${selectedAgent.name} ${selectedAgent.familyName}`
      : selectedDeadAgent !== null
        ? `${selectedDeadAgent.name} ${selectedDeadAgent.familyName}`
        : 'The Watched';

  const atlasSwitcher = (
    <div>
      <div style={{ marginBottom: 7 }}>
        <Kicker>Map layout</Kicker>
      </div>
      <Segmented
        value={mobileAtlas}
    ...
    <div style={{ background: c.base, color: c.text, height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {vp.isMobile ? (
        <AtlasMobile
          ...
          panel={panelNode}
          caption={captionNode}
          ledgerLabel="The Watched"
          soulCount={worldSnapshot?.population ?? 0}
        ...
        <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0, padding: '20px 22px', display: 'flex', flexDirection: 'column' }}>
            {mapFrame({ position: 'relative', flex: 1, minHeight: 0, border: `1px solid ${c.lineStrong}`, boxShadow: '0 2px 24px rgba(0,0,0,0.45)' }, false)}
            {captionNode}
          </div>

          {/* ledger / inspector */}
          <aside style={{ width: 360, flexShrink: 0, background: c.panel, borderLeft: `1px solid ${c.lineStrong}`, overflowY: 'auto', padding: '22px 22px 40px' }}>
            {panelNode}
          </aside>
        </div>
      )}
